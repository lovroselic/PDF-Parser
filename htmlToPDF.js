...
(() => {
    ...
    async function renderPageToPDF(doc, page, imgWidth) {
        try {
            ...
            //console.log("page", page, "imgHeight", imgHeight);
    ...
    function pageFooter(doc, page, totalPages) {
        const datum = new Date().stringify();
        doc.text(`Page: ${page}/${totalPages}; Generated by ChestBook ${datum}, coded by LS; v${INI.VERSION}`, 10, doc.internal.pageSize.height - 10);
    }
    ...
    function pageLine(doc) {
        ...
        /* logo */
        const logo = $('#logo')[0];
        const logoPosY = doc.internal.pageSize.height - INI.LOGO_UP;
    ...
    function plainChecklist(title, checklist) {
        const id = stringToUnderscoredString(title);
        let element = `<div class="row mb-3" id="${id}"><p class="fw-bold">${title}</p>`;
        element += add_checklist(checklist);
        element += "</div>";
        return element;
    }
    ...
    function formativnoSpremljanje(page, chapter) {
        $(page).append(`<h2 class="my-4">${chapter}. Formativno spremljanje</h3>`);
    ...
    function oblikaDela(page, chapter) {
        ...
        $(page).append(`<h4 class="my-2">${chapter} Oblika dela</h4>`);
    ...
    function doodling() {
        ...
        $("#page-5").append(dualTextArea("THE SIX SENTENCES checklist", checklist, "the sentences", "activity, potek dela"));
        $("#page-5").append(`<h3 class= "my-3">2.2.1 Jezik</h3>`);
        ...
        for (const [index, goal] of cilji.entries()) {
            ...
            $("#page-5").append(element);
        }

        oblikaDela("#page-5", "2.2.1");
    }
    ...
    function elementarneIgre() {
        ...
        const checklist = [
            ["UVOD", "JEDRO", "ZAKLJUČEK", "Elementarna igra", "Učenje besedišča", "Učenje dialoga", "Učenje slovnice"],
            ["Globalno: razumevanje glavne ideje zgodbe", "Selektivno: prepoznavanje detajlov (v zgodbi)", "Funkcionalno: razumevanje navodil"],
    ...
    function zakljucnaRutina() {
        ...
        for (const [index, title] of titles.entries()) {
            ...
            $("#page-99").append(element);
        }
    }

    function first_page() {
        const cilji = ["Namen učne ure", "Cilji po učnem načrtu"];
        const checklist = [
            ["Preverjanje", "Končno preverjanje", "Ocenjevanje", "Utrjevanje", "Obravnava nove snovi", "Analiza"],
            ["Poslušanje ali slušno razumevanje", "Govorno sporočanje", "Sporazumevanje", "Posredovanje (mediacija)", "Branje in bralno razumevanje", "Pisanje in pisno sporočanje"],
        ];

        for (const [index, cilj] of cilji.entries()) {

            const element = plainChecklist(cilj, checklist[index]);
            $("#front_page").append(element);
        }
    }
    ...
    function build() {
        ...
        */
        ...
        uporabaJezika();
        zakljucnaRutina();
        formativnoSpremljanje("#page-99", 4);
    }
